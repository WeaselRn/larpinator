  ...
  if (input.file) {
    ...
    if (type === "application/pdf") {
      if (file.size > MAX_PDF_BYTES) {
        throw new Error("PDF is too large. Max size is 10MB.");
      }
    ...
    if (type.startsWith("image/")) {
      ...
      if (!text || text.length < 40) {
        throw new Error("Could not read enough text from that image. Try a clearer screenshot or paste the text.");
      }
      return { text: text.slice(0, MAX_TEXT_CHARS), source: "image" };
    }

    throw new Error("Unsupported file type. Upload a PDF or image, or paste your resume text.");
  }

  const text = (input.text ?? "").trim();
  if (!text) {
    throw new Error("Nothing to analyze. Upload a file or paste your resume text.");
  }
  if (text.length < 80) {
    throw new Error("That's not a resume, that's a tweet. Give me more to roast.");
  }
...
export async function runGithubAnalysis(profile: Profile, usernameInput: string): Promise<AnalysisOutcome> {
  ...
  if (!username) {
    throw new Error("That doesn't look like a GitHub username or profile URL.");
  }
...
export async function runMusicAnalysis(profile: Profile, usernameInput: string): Promise<AnalysisOutcome> {
  ...
  if (!username) {
    throw new Error("That doesn't look like a Last.fm username or profile URL.");
  }
...
export async function runCombinedAnalysis(profile: Profile): Promise<AnalysisOutcome> {
  ...
  if (sources.length < 2) {
    throw new Error(
      "Combined analysis needs at least 2 completed analyses (CV, GitHub, Music, Quiz, Daily or Battle). Go collect some evidence first.",
    );
  }
...
export async function runDailyAnalysis(
  ...
  if (text.length < 5) {
    throw new Error("Write at least a few words. Dodging with silence is too meta.");
  }
  if (text.length > 2000) {
    throw new Error("Keep it under 2000 characters. Brevity is aura.");
  }
...
export async function runBattleAnalysis(profile: Profile, input: BattleInput): Promise<BattleOutcome> {
  ...
  if (playerOneText.length < 60 || playerTwoText.length < 60) {
    throw new Error("Both players need at least 60 characters of CV/profile material to battle with.");
  }
  if (playerOneText.length > 8000 || playerTwoText.length > 8000) {
    throw new Error("Keep each submission under 8000 characters.");
  }
